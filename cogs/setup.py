...
class Setup(commands.Cog):
    ...
    async def setup_youtube(self, ctx):
        ...
        try:
            ...
            embed.add_field(
                name="Nastavení",
                value="ID kanálu bylo automaticky přidáno do .env souboru."
            )
    ...
    async def setup_all(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description="Všechny kanály byly úspěšně vytvořeny a nastaveny.",
            color=discord.Color.green()
        )
        
        embed.add_field(
            name="Důležité",
            value="Nezapomeňte nastavit následující proměnné v .env souboru:\n"
                  "- DISCORD_TOKEN\n"
                  "- GUILD_ID\n"
                  "- WELCOME_CHANNEL_ID\n"
                  "- YOUTUBE_API_KEY\n"
