...
class ChatSummary(commands.Cog):
    ...
    async def generate_summary(self, messages, date):
        ...
        system_prompt = (
            ...
            "Tvým úkolem je analyzovat zprávy a vytvořit poutavé a čtivé shrnutí hlavních témat, "
            "která se v chatu probírala. Shrnutí by mělo být v češtině a mělo by být rozděleno "
            "podle témat, ale neformálním a zábavným způsobem. Pro každé téma identifikuj první zprávu, "
            "která toto téma začíná, a označ ji jako '[TOPIC_START:index]', kde index je pořadové číslo zprávy "
            "v seznamu (začíná od 0). Zahrň odkazy na zprávy, které jsou v textu (začínají http:// nebo https://). "
            "Místo suchého reportu se zaměř na to, aby shrnutí bylo zábavné a poutavé, jako bys vyprávěl/a příběh. "
            "Formátuj datum v hlavičce správně - DD. M. YYYY znamená den, měsíc a rok s tečkami (např. 18. 4. 2025)."
        )

        user_prompt = (
            f"Zde jsou zprávy z Discord chatu ze dne {date}. Vytvoř zábavné a poutavé shrnutí "
            f"hlavních témat, která se probírala. Rozděl shrnutí podle témat, ale piš to jako bys vyprávěl/a příběh, "
            f"ne jako suchý report. Pro každé téma identifikuj první zprávu, která toto téma začíná, a označ ji jako '[TOPIC_START:index]'. "
            f"Nezapomeň zahrnout odkazy, pokud se v chatu nějaké vyskytly. Na konci shrnutí přidej krátký a vtipný "
            f"souhrn celého dne, a pokud se v chatu událo něco obzvlášť vtipného, nezapomeň to zmínit! "
